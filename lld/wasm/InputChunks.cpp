...
void InputChunk::relocate(uint8_t *buf) const {
  ...
  for (const WasmRelocation &rel : relocations) {
    ...
    // R_WASM_TYPE_INDEX_LEB and R_WASM_FUNCTION_OFFSET_LEB don't have to have symbol names
    if (rel.Type != R_WASM_TYPE_INDEX_LEB && rel.Type != R_WASM_FUNCTION_OFFSET_LEB)
